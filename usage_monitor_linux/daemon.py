...
Owns every decision the desktop frontends do not make themselves.

Polling cadence, reset detection, threshold alerts, event commands and
notifications all live here, exactly as they do in the Windows tray
application.  The panel widget receives a finished snapshot and draws it.

The polling schedule is deliberately not a timer: it is a loop that
aligns itself to the next quota reset, backs off on rate limits, pauses
while the session is locked, and never fetches inside the danger window
before a reset.  A naive periodic timer would break all four properties.
"""
...
from usage_monitor_for_claude.anthropic_status import AnthropicStatus, AnthropicStatusCache
from usage_monitor_for_claude.api import read_access_token
from usage_monitor_for_claude.cache import UsageCache
from usage_monitor_for_claude.command import run_event_command
from usage_monitor_for_claude.formatting import elapsed_pct, field_period, format_credits, parse_field_name, popup_label
from usage_monitor_for_claude.i18n import T
from usage_monitor_for_claude.settings import (
    ALERT_EXTRA_USAGE_SPENT, ALERT_TIME_AWARE, ALERT_TIME_AWARE_BELOW, ICON_FIELDS, IDLE_PAUSE,
    NOTIFY_CLAUDE_UPDATE, ON_DOUBLE_CLICK_COMMAND, ON_RESET_COMMAND, ON_STARTUP_COMMAND, ON_THRESHOLD_COMMAND,
    POLL_ERROR, POLL_FAST, POLL_FAST_EXTRA, POLL_INTERVAL, STATUS_ENABLED, STATUS_NOTIFICATIONS, STATUS_POLL_INTERVAL,
...
__all__ = ['RESET_BUFFER', 'UsageMonitorDaemon', 'align_to_reset']
...
# Seconds after a reset that a confirming poll aims for.
RESET_BUFFER = 5


def align_to_reset(interval: int, next_reset: float | None) -> tuple[int, bool]:
    ...
    if next_reset < interval + danger:
        ...
        # the window start; if that is too close to keep POLL_FAST spacing,
        # commit to the confirming poll directly.
...
class UsageMonitorDaemon:
    """Polls the usage API and publishes snapshots to the desktop frontends.

    Parameters
    ----------
    publish : callable
        Called with the current snapshot dict whenever the data changes.
        The callee is responsible for marshalling onto the D-Bus main
        loop thread - the poll loop runs on its own thread.
    """
    ...
    def __init__(self, publish: Callable[[dict[str, Any]], None]) -> None:
        ...
        # Guarded by _notify_lock: deferrals arrive from the D-Bus and poll
        ...
        # Set by a Refresh() call over D-Bus.  The request is only recorded
        # here so the caller returns immediately: a fetch on the main loop
        # thread would block every other D-Bus method for its duration.
    ...
    def snapshot(self) -> dict[str, Any]:
        """Return the current snapshot payload."""
        return build_snapshot(
            self.cache.snapshot, next_poll_time=self._next_poll_time,
            anthropic_status=self._anthropic_status,
            failed='error' in self._last_response,
            auth_error=bool(self._last_response.get('auth_error')),
        )

    def request_refresh(self) -> bool:
        """Record an out-of-band refresh request, returning False when idle-paused.

        The poll loop performs the fetch; a request made while the session
        is locked is declined rather than queued, matching the rule that
        polling stops entirely during idle and lock.
        """
    ...
    def run_event_test(self, event: str) -> bool:
        """Run one configured event command with representative values.

        Mirrors the Windows tray's "Test event commands" menu: the samples
        below are fixed so a user can see what their command receives
        without waiting for a real reset or threshold crossing.  These are
        user-driven, so failures surface (``capture_output``) instead of
        being swallowed the way automatic events are.

        Parameters
        ----------
        event : str
            One of ``reset_5h``, ``reset_7d``, ``threshold_5h``,
            ``threshold_7d``, ``startup`` or ``double_click``.

        Returns
        -------
        bool
            False when no command is configured for that event, or the
            name is unknown.
        """
        samples = {
            'reset_5h': (ON_RESET_COMMAND, {
            ...
            'threshold_5h': (ON_THRESHOLD_COMMAND, {
            ...
            'threshold_7d': (ON_THRESHOLD_COMMAND, {
                'USAGE_MONITOR_EVENT': 'threshold',
                'USAGE_MONITOR_VARIANT': 'seven_day',
            ...
            'double_click': (ON_DOUBLE_CLICK_COMMAND, {
                'USAGE_MONITOR_EVENT': 'double_click',
                **self._quota_snapshot_env(self._last_response),
            }),
        }

        sample = samples.get(event)
        if sample is None or not sample[0]:
            return False

        run_event_command(sample[0], sample[1], capture_output=True)

        return True

    def run_double_click_command(self) -> bool:
        """Run the configured double-click command against current usage."""
    ...
    def update(self, force: bool = False) -> None:
        ...
        self._check_reset(quota_fields, result.data)
    ...
    def _check_reset(self, quota_fields: dict[str, float], data: dict[str, Any]) -> None:
        """Notify and run the reset command when a quota drops.

        The notification only fires for a quota that was nearly exhausted
        and is not masked by another blocking quota; the command fires on
        any drop.  The message carries no field information, so several
        quotas resetting within one polling gap produce a single
        notification.
        """
        ...
        for key, pct in quota_fields.items():
            ...
            if prev is not None and pct < prev:
                self._run_reset_command(key, pct, prev, data=data, entry=data.get(key, {}))
                self._idle_reset_pending = False

    def _refresh_anthropic_status(self) -> None:
        """Refresh the Anthropic status indicator and react to state changes.

        An ``unknown`` reading (feed unreachable) is shown in the popup but
        is never treated as a state change - a flaky connection cannot fire
    ...
    def _flush_deferred_notifications(self) -> None:
        ...
        mid-flush is kept for the next flush instead of mutating the dict
        being iterated.
        """
        with self._notify_lock:
    ...
    def _check_threshold_alerts(self, data: dict[str, Any]) -> None:
        ...
        Dynamically detects all quota fields in the API response.  For each
        field, finds the highest threshold exceeded by current utilization.
        When usage drops (e.g. after reset), tracking resets so thresholds
        can re-trigger in the next cycle.
        """
        for variant_key, entry in data.items():
            ...
            if highest_exceeded > last_notified:
                ...
                message = T['notify_threshold_generic'].format(label=popup_label(variant_key), pct=f'{pct:.0f}')
    ...
    def _check_extra_usage_spent_alerts(self, extra: dict[str, Any], used: float, used_text: str) -> None:
        ...
        if highest_exceeded > last_notified:
            ...
            self._run_threshold_command('extra_usage_spent', None, highest_exceeded, extra, title, message, extra_used=used_text)
    ...
    def _quota_snapshot_env(self, data: dict[str, Any]) -> dict[str, str]:
        ...
        ``USAGE_MONITOR_EXTRA_LIMIT`` when it also has a monthly limit.
        """
    ...
    def _run_startup_command(self, data: dict[str, Any]) -> None:
        ...
        Fires once after the first successful API update.  Receives the full
        quota state so the command can decide what to do.
        """
        if not ON_STARTUP_COMMAND:
            return

        run_event_command(ON_STARTUP_COMMAND, {'USAGE_MONITOR_EVENT': 'startup', **self._quota_snapshot_env(data)})

    def _run_reset_command(self, variant: str, pct: float, prev_pct: float, *, data: dict[str, Any], entry: dict[str, Any]) -> None:
    ...
    def _run_threshold_command(
        self, variant: str, pct: float | None, threshold: float, entry: dict[str, Any], title: str, message: str,
        *, extra_used: str = '', extra_limit: str = '',
    ) -> None:
        """Run the user-configured threshold command if set.

        Skipped on the first update so that already-exceeded thresholds at
        startup do not trigger commands.  Notifications still fire - commands
        react to *events*, not *state*.
        ...
        env_vars = {'USAGE_MONITOR_EVENT': 'threshold', 'USAGE_MONITOR_VARIANT': variant}
        if pct is not None:
    ...
    def _account_switched(self) -> bool:
        ...
        Probes the account profile with the token now in the credentials file
        (bypassing the 429 backoff, since a freshly selected account cannot be
        the source of that rate limit) and compares its UUID against the last
        seen one.  Returns False until a baseline UUID is known, so the first
        successful update is never taken for a switch.
        """
    ...
    def _calculate_poll_interval(self) -> int:
        ...
        interval, aligned = align_to_reset(interval, next_reset)
        if aligned:
    ...
    def _is_user_away(self) -> bool:
        """Return True if the user is idle or the session is locked."""
        if is_workstation_locked():
            return True
        return IDLE_PAUSE > 0 and get_idle_seconds() >= IDLE_PAUSE

    def _wait_for_activity(self, until: float | None = None) -> None:
        """Block until user activity resumes or the daemon is stopping.

        Parameters
        ----------
        until : float | None
            Optional deadline (``time.time()`` epoch).  When set, the wait
            ends even if the user is still away, allowing a time-critical
            poll (e.g. quota reset command) to proceed.
        """
    ...
    def run(self) -> None:
        ...
        Pauses polling when the user is idle or the session is locked.  On
        resume, polls immediately if the regular interval has elapsed since
        ...
        while self.running:
            ...
            # Re-publish now that the next poll is scheduled: the snapshot sent
            # from update() was built before the interval was known, so its
            # next_poll_time was still empty.
            self._publish(self.snapshot())
            ...
            while self.running and time.time() < target:
                ...
                # up with the pre-jump target, potentially for hours.
                if target - time.time() > interval + POLL_FAST:
                    target = time.time() + interval
                    self._next_poll_time = target

                # If another path fetched successfully, push the next poll a
                # full interval past that fetch to avoid a redundant one.  Only
                # react to an actual new fetch (last_success advanced).
                latest_success = self.cache.last_success_time
                if latest_success is not None and (last_success_seen is None or latest_success > last_success_seen):
                    last_success_seen = latest_success
                    new_target = max(target, latest_success + interval)
                ...
                # Pause polling while the user is away.  The only exception:
                # when on_reset_command is configured and a quota reset is due,
                # the idle pause is interrupted so the command fires on time.
                # _idle_reset_pending keeps polling at POLL_INTERVAL until the
                # reset is actually confirmed (usage drop), covering server-side
                # delays and transient network errors.
                if self._is_user_away():
                    reset_deadline = None
                    if ON_RESET_COMMAND:
                        next_reset = self._seconds_until_next_reset()
                    ...
                    # User returned - show any notifications deferred during
                    # idle and poll immediately if the interval elapsed.
                    # _idle_reset_pending is intentionally kept: if the user
                    # locks again before a successful poll confirms the reset,
                    # idle polling must resume.
                    self._flush_deferred_notifications()
                    latest_success = self.cache.last_success_time
                    ...
                    if time.time() - latest_success >= interval:
                        break
