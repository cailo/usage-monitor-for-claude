...
## Claude CLI
- The `cli_command` setting (name -> base command, e.g. a WSL install) **replaces** the auto-detected native binary rather than supplementing it, and does so for all three consumers at once: version display (`find_installations`), token refresh (`refresh_token`), and the API User-Agent (`active_cli_version`). Listing it alongside the native CLI would leave refresh and the User-Agent pointed at the wrong (often stale) binary - which is the bug the setting exists to fix. IDE extension detection is independent and stays untouched
- `cli_version()` caches per binary **mtime**, so an update is picked up automatically. A custom `cli_command` has no local file to stat (`wsl ...` cannot be mapped to a Windows path reliably), so `_command_version()` caches per command tuple for the process lifetime instead. That cache has no automatic invalidation: `refresh_token()` must drop the entry whenever it installs an update, otherwise the stale version sticks until restart. Never cache a failed run (timeout/OSError) - that would pin an empty version for the whole session
- Both version helpers run through `_parse_version()`. Never write an unparsed string into a version cache: it flows into the API User-Agent (`claude-code/<version>`) and the popup
