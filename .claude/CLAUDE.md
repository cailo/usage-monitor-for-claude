...
## Platform
- Windows-first application with a Linux port. `usage_monitor_for_claude/` holds the shared core **and** the Windows UI, `usage_monitor_linux/` holds the headless Linux daemon, and `frontends/` holds one drawing surface per desktop: `frontends/plasma/` (QML applet) and `frontends/gnome/` (GJS shell extension) - `docs/linux.md` covers the port in detail
- A frontend only draws. Polling, formatting, translation, alerts and the credentials all live in the daemon, which publishes a finished snapshot over D-Bus - no token ever enters a desktop shell process. A new frontend never gains a second job: if it needs a formatting rule or a label, that belongs in `snapshot.py`, not in QML or JavaScript
- Dependency direction is one-way: `usage_monitor_linux` imports from `usage_monitor_for_claude`, never the reverse. An import from the core into the Linux package is an architecture error, not a shortcut
- The Windows UI/OS modules (`app.py`, `tray_icon.py`, `popup.py`, `notification_identity.py`, `autostart.py`, `idle.py`, `single_instance.py`, `verbose.py`, `__main__.py`) may use `ctypes.windll` and `winreg` unconditionally - the Linux daemon never imports them and ships its own equivalents in `usage_monitor_linux/`
- The core modules (`api.py`, `cache.py`, `settings.py`, `i18n.py`, `formatting.py`, `command.py`, `claude_cli.py`, `anthropic_status.py`) are imported by the Linux daemon, so they must never call a Windows API directly. Every divergence goes through `platform_support.py` (`IS_WINDOWS`, `error_box`, `no_window_flags`, `desktop_ui_language`, `system_time_format`). A Windows-only call added here does not fail the Windows build - it breaks Linux silently at runtime
- `platform_support.py` is the single place that branches on platform, and it lives in the core rather than in `usage_monitor_linux/` because core modules consume it - moving it there would invert the dependency
...
- Every subprocess call in `claude_cli.py` goes through `_run_cli()`, which pins `encoding='utf-8', errors='replace'`. The Claude CLI is a Node application and writes UTF-8 whatever the ambient locale codec is; letting `subprocess` decode with that codec instead breaks differently on each platform, and both ways are fatal: on Windows the decode raises inside the reader thread `subprocess` uses to drain the pipe and the stream comes back as `None` despite `capture_output=True` (per-stream, so an ASCII version number next to a non-ASCII npm warning leaves stdout intact and drops only stderr), while on POSIX the decode happens in the calling thread and raises `UnicodeDecodeError` - a `ValueError`, which `refresh_token()`'s `except OSError` does not catch, so a daemon started with a non-UTF-8 locale (`LANG=C`) loses its poll thread. `_run_cli()` passes `creationflags=no_window_flags()`, never `subprocess.CREATE_NO_WINDOW` - that constant does not exist on Linux. Do not add a bare `subprocess.run` here, and do not extend this to `command.py` - that one runs arbitrary user commands, where a `.bat` emits the OEM code page and the ambient codec is the correct assumption
