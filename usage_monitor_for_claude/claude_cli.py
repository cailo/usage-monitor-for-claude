...
from .platform_support import IS_WINDOWS, no_window_flags
...
def _discover_cli_path() -> Path:
    ...
    Strategy
    --------
    0. On Linux, ``shutil.which('claude')`` followed by the usual install
       locations.  There are no executable suffixes to resolve there, so
       the Windows shim handling below does not apply.
    ...
    """
    if not IS_WINDOWS:
        return _discover_posix_cli_path()
...
def _discover_posix_cli_path() -> Path:
    """Discover the Claude Code CLI binary path on Linux.

    Checks ``PATH`` first, then the locations the native installer and a
    global npm install use.  Falls back to the native installer path so
    callers' ``is_file()`` checks fail gracefully.
    """
    found = shutil.which('claude')
    if found:
        return Path(found)
