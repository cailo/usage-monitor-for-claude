...
3. ``$XDG_CONFIG_HOME/usage-monitor-for-claude/usage-monitor-settings.json`` (Linux only)
...
from .platform_support import IS_WINDOWS, error_box, system_time_format
...
__all__ = [
    ...
    'STATUS_ENABLED', 'STATUS_NOTIFICATIONS', 'STATUS_POLL_INTERVAL',
...
_BOOL_KEYS = frozenset({'alert_time_aware', 'notify_claude_update', 'status_enabled', 'status_notifications'})
...
def _load_settings() -> dict:
    ...
    xdg_config = Path(os.environ.get('XDG_CONFIG_HOME') or Path.home() / '.config') / 'usage-monitor-for-claude'
    ...
    if not IS_WINDOWS:
        search_paths.append(xdg_config / SETTINGS_FILENAME)
...
# Anthropic server status indicator (status.anthropic.com)
STATUS_ENABLED: bool = _S.get('status_enabled', True)
STATUS_POLL_INTERVAL: int = _S.get('status_poll_interval', 300)
STATUS_NOTIFICATIONS: bool = _S.get('status_notifications', True)
