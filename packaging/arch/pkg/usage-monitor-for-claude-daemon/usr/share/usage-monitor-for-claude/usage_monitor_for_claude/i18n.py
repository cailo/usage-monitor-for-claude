...
Loads translations for the detected interface language with English fallback.
"""
...
from .platform_support import desktop_ui_language
...
def load_translations() -> dict[str, Any]:
    """Load translations for the configured or detected interface language, fallback to English.

    The desktop's interface language takes precedence over the locale
    reported by ``locale.getlocale()``, which describes the regional
    formats and can name a different language entirely.
    """
    ...
    lang = desktop_ui_language() or locale.getlocale()[0] or ''
