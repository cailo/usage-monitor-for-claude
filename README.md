...
- **Anthropic status indicator** - know instantly whether a hiccup is on your end or Anthropic's: the popup shows the live status of the Anthropic servers ([status.anthropic.com](https://status.anthropic.com)) as a colored dot with the reported status text, the tray tooltip warns while an incident is ongoing, and a notification fires when an incident starts and when it is resolved. [Can be turned off entirely](docs/configuration.md#anthropic-status) if you prefer the strict single-host setup
...
- **Two read-only network destinations** - your credentials and usage data go exclusively to `api.anthropic.com`. The only other host is `status.anthropic.com`, the public Anthropic status feed for the [status indicator](docs/configuration.md#anthropic-status): read-only, no credentials involved, and fully removable with `status_enabled: false` for a strict single-host setup
...
- **Writes no files** - the app never writes files to disk. The only lasting changes are the autostart entry you enable yourself (two `HKEY_CURRENT_USER` registry values on Windows, one `~/.config/autostart/` entry on Linux). On Windows those two values are: the toast notification identity (display name and icon, re-registered on every start) and the autostart entry, written only when you enable autostart. An expired OAuth token additionally triggers `claude update`, which may install a newer Claude Code version
...
- **Minimal runtime dependencies** - only four well-known packages: [requests](https://pypi.org/project/requests/), [Pillow](https://pypi.org/project/pillow/), [pystray](https://pypi.org/project/pystray/), [pywebview](https://pypi.org/project/pywebview/). The Linux build needs fewer still: the panel frontend is drawn by the desktop shell itself, so it drops Pillow, pystray and pywebview and adds only python-dbus

---

## Requirements

- **Windows 10 or Windows 11** (64-bit), or **Linux with KDE Plasma 6 or GNOME 45+** - see [docs/linux.md](docs/linux.md)
