...
- **Claude Code versions** - the popup shows which version is installed in each environment (native CLI, VS Code, Cursor, Windsurf), making it easy to spot when your IDE extension is ahead of or behind the CLI. Run Claude Code under WSL? Point the version check at it with the [`cli_command`](docs/configuration.md#claude-cli-command) setting so you see the version you actually use
